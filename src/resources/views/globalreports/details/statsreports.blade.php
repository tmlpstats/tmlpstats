        ...
        <table id="activeCenterTable" class="table">
            ...
            @foreach ($statsReportsList as $data)
                <tr id="{{ $data['id'] }}"
                    ...
                    <td><a href="{{ url("/statsreports/{$data['officialReport']->id}") }}">{{ $data['officialSubmitTime'] }}</a></td>
                    @if (isset($data['revisedReport']))
                    <td style="text-align: center">Yes</td>
                    <td><a href="{{ url("/statsreports/{$data['revisedReport']->id}") }}">{{ $data['revisionSubmitTime'] }}</a></td>
                    @else
