...
class ContactInfoImporter extends DataImporterAbstract
{
    ...
    protected function loadEntry($row, $unused = null)
    {
        ...
        if (strtoupper($nameParts['firstName']) == $nameParts['firstName']) {
            $this->addMessage("First name is in all capital letters. Please provide them with appropriate capitalization, otherwise we may not be able to find this person in some cases.", 'warn', $row);
        }
